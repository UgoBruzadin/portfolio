      ...
      <div className="space-y-6">
        ...
        >
          {[
            ...
            { value: '1.6M+', label: 'EEG trials collected' },
            { value: '+14.8%', label: 'Cross-subject accuracy' },
            { value: '5+ TB', label: 'Neural data managed' },
          ].map(({ value, label }) => (
            <div
              key={label}
              className="rounded-xl border border-slate-200 dark:border-slate-700/60 bg-slate-50 dark:bg-slate-800/40 px-4 py-3 text-center"
            >
              <div className="text-xl font-bold text-neural-500 dark:text-neural-400 tabular-nums">
                {value}
              </div>
